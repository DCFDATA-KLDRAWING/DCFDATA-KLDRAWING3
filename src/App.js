...
/**
 * 版本 v5.4 更新日誌 (100% 綠燈純淨版)：
 * 1. 深度潔癖淨化：徹底刪除遺留的 `getTrend` 函數與 `maTrend` 變數，消除 no-unused-vars 警告。
 * 2. 嚴格模式修正：為所有的 catch {} 補上 console 輸出，消除 no-empty 警告。
 * 3. 確保無任何 ESLint 警告，完美迎合 Vercel 最嚴格的打包標準。
 */
...
// --- 外部純函數 (只保留最穩定的單一價格抓取) ---
const fetchPriceFromYahoo = async (symbol) => {
  try {
    ...
    const tryFetch = async (suffix) => {
      const url = `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}${suffix}?interval=1d&range=1d&_t=${Date.now()}`;
    ...
    let price = await tryFetch(".TW");
    if (!price) price = await tryFetch(".TWO");
    return price;
  } catch (e) {
...
const App = () => {
  ...
  const updateStockData = useCallback(async (stockName) => {
    ...
    const latestPrice = await fetchPriceFromYahoo(stockCode);
    if (latestPrice) {
...
`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col text-slate-800 selection:bg-indigo-100 selection:text-indigo-900">
      {/* --- 頂部標題列 --- */}
      <header className="bg-gradient-to-r from-slate-900 to-slate-800 text-white shadow-lg sticky top-0 z-40 px-5 py-3 flex justify-between items-center">
        <h1 className="text-[15px] font-black flex items-center gap-2.5 tracking-wide">
          <img
            src="https://lh3.googleusercontent.com/d/18d55eFbuwNKnqA_p8ObEbNo8xDkpnvcU"
            alt="My Logo"
            className="h-1 w-auto object-contain rounded drop-shadow-md"
          />
          DCF 估值模型{" "}
          <span className="opacity-40 font-normal hidden md:inline">|</span>{" "}
          <span className="text-emerald-400 text-xs tracking-normal hidden md:inline">
            v5.4 Pro
          </span>
        </h1>
        <button
          onClick={handleReset}
          className="text-xs font-bold text-slate-300 hover:text-white hover:bg-slate-700/50 transition-all flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-600/30"
        >
          <RefreshCw size={13} /> 重新重置
        </button>
      </header>
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-6 space-y-6">
        {/* --- 卡片 1：核心參數區 --- */}
        <section className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-5 md:p-6 relative">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 relative">
            ...
            <div className="md:col-span-4 relative" ref={dropdownRef}>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50/50 border border-slate-200 rounded-xl pl-9 pr-3 py-2.5 text-sm font-bold outline-none focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all"
                  placeholder="搜尋台股..."
                />
                <Search
                  size={15}
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-full bg-white border border-slate-100 rounded-xl shadow-2xl shadow-slate-900/10 z-50 max-h-60 overflow-y-auto">
                  {filteredStocks.length > 0 ? (
                    <ul className="py-1.5">
                      {filteredStocks.map((stock) => (
                        <li
                          ...
                          className="px-4 py-2.5 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 cursor-pointer border-b border-slate-50 last:border-0 font-semibold transition-colors"
                        >
                          {stock}
                        </li>
                      ))}
                    ...
                    <div className="px-4 py-4 text-xs text-slate-500 text-center bg-slate-50 rounded-xl m-1">
                      查無台股資料
                      <br />
                      可直接自填名稱
                    </div>
            ...
            <div className="md:col-span-3">
              <label className="text-xs font-bold text-indigo-600 block mb-1.5 ml-1">
                基準 EPS
              </label>
              <input
                type="number"
                ...
                className="w-full bg-indigo-50/30 border border-indigo-200 rounded-xl px-3 py-2.5 text-sm font-black text-indigo-700 text-center outline-none focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-inner shadow-indigo-900/5"
                placeholder="0"
              />
            </div>
            ...
            <div className="md:col-span-5 flex flex-col">
              <div className="flex justify-between items-center mb-1.5 ml-1">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] text-slate-400 hover:text-indigo-600 transition-colors disabled:opacity-50 font-bold bg-slate-100 px-2 py-0.5 rounded-md"
                  title="重新抓取最新股價"
                >
                  <RefreshCw
                    size={10}
                  ...
                  />
                  {isFetchingPrice ? "抓取中" : "抓取最新"}
                </button>
              </div>

              <div className="flex h-[42px] relative shadow-sm rounded-xl overflow-hidden group focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all">
                <input
                  type="number"
                  step="any"
                  value={isFetchingPrice ? "" : price}
                  onChange={handleInputChange(setPrice)}
                  className="w-full h-full bg-slate-50/50 border border-slate-200 border-r-0 px-3 text-sm font-black text-slate-800 text-center outline-none focus:bg-white focus:border-indigo-500 disabled:bg-slate-100 disabled:text-slate-400 placeholder:text-indigo-300"
                ...
                <button
                  ...
                  className="h-full bg-indigo-600 hover:bg-indigo-500 text-white px-4 flex items-center justify-center shrink-0 transition-colors disabled:bg-slate-300"
                  title="依股價反推隱含成長率"
                >
                  {isSolving ? (
          ...
          <div className="mt-5 pt-5 border-t border-slate-100">
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold transition-all duration-300 text-xs sm:text-sm ${
                showAssistant
                  ? "bg-slate-800 text-white shadow-inner"
                  : "bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 shadow-sm hover:shadow"
              }`}
            >
              <Sparkles
                size={16}
              ...
              />
              {showAssistant ? "收起情報小助理" : "展開 AI 產業情報小助理"}
              {showAssistant ? (
            ...
            {showAssistant && (
              <div className="mt-3 p-4 md:p-5 bg-slate-800 rounded-2xl shadow-inner border border-slate-700 animate-in slide-in-from-top-2 fade-in duration-200">
                <div className="flex items-center gap-2 mb-4">
                  <Bot size={18} className="text-amber-400 drop-shadow" />
                ...
                {/* 專家專區按鈕 */}
                <div className="flex flex-wrap gap-2.5 mb-4 pb-4 border-b border-slate-600/50">
                  <button
                    onClick={handleSupplyDemandPrompt}
                    className="px-4 py-2 rounded-xl border border-blue-500/50 bg-gradient-to-r from-blue-600 to-blue-700 text-white text-xs font-bold hover:shadow-lg hover:shadow-blue-900/50 transition-all flex items-center gap-1.5"
                  >
                    <Target size={14} /> 供需派產業觀察師
                  </button>

                  <button
                    onClick={handleComprehensiveGrowthPrompt}
                    className="px-4 py-2 rounded-xl border border-purple-500/50 bg-gradient-to-r from-purple-600 to-purple-700 text-white text-xs font-bold hover:shadow-lg hover:shadow-purple-900/50 transition-all flex items-center gap-1.5"
                  >
                    🎯 綜合評估成長率
                  </button>
                </div>
                  ...
                  {ASSISTANT_TOPICS.map((topic, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg border text-xs transition-all active:scale-95 text-left leading-snug ${
                          isLongTopic ? "w-full" : ""
                        } ${
                          isSelected
                            ? "bg-amber-500 text-slate-900 border-amber-400 font-bold shadow-md shadow-amber-900/20"
                            : "border-slate-600 bg-slate-700/50 text-slate-200 hover:border-amber-400 hover:bg-slate-700"
                        }`}
                      >
                        {topic}
                      </button>
                ...
                <div className="flex flex-col gap-3 pt-4 border-t border-slate-600/50">
                  <div className="flex justify-between items-center bg-slate-900/50 rounded-lg p-3 border border-slate-700/50">
                    <div className="flex flex-wrap items-center gap-1.5 text-xs text-slate-300 font-medium">
                      ...
                      {activeTopics.length > 0 ? (
                        activeTopics.map((topic, idx) => (
                          <span key={idx} className="inline-flex items-center">
                            ...
                            {idx < activeTopics.length - 1 && (
                              <span className="text-slate-600 text-[10px] mx-1.5">
                                +
                              </span>
                            )}
                          </span>
                        ...
                        <span className="text-slate-500 italic">尚未選擇</span>
                      )}
                    </div>
                    {activeTopics.length > 0 && (
                      <button
                        onClick={() => setActiveTopics([])}
                        className="text-[10px] text-slate-400 hover:text-rose-400 transition-colors font-bold shrink-0 ml-3 bg-slate-800 px-2.5 py-1 rounded-md border border-slate-700"
                      >
                        清除
                      </button>
                    )}
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-3">
                    <select
                      ...
                      className="w-full md:w-auto flex-none bg-slate-700 text-white text-xs border border-slate-600 rounded-xl px-3 py-3 outline-none focus:border-amber-500 cursor-pointer shadow-sm"
                    >
                    ...
                    <div className="flex w-full gap-2.5">
                      <button
                        ...
                        className="flex-1 bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold text-xs py-3 rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-md shadow-amber-900/20"
                      >
                        <Search size={15} /> Google
                      </button>
                      <button
                        onClick={handlePerplexitySubmit}
                        className="flex-1 bg-gradient-to-br from-[#1A8276] to-[#136359] hover:brightness-110 text-white font-bold text-xs py-3 rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-md shadow-[#1A8276]/20"
                      >
                        <BrainCircuit size={15} /> Perplexity
                      </button>
                      <button
                        onClick={handleChatGPTSubmit}
                        className="flex-1 bg-gradient-to-br from-[#10a37f] to-[#0c8265] hover:brightness-110 text-white font-bold text-xs py-3 rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-md shadow-[#10a37f]/20"
                      >
                        <MessageCircle size={15} /> ChatGPT
                      </button>
                    </div>
                  </div>
                </div>
        ...
        {/* --- 卡片 2：成長率設定區 --- */}
        <section className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-5 md:p-6 space-y-5">
          {/* 1-5 年區塊 */}
          <div className="bg-slate-50/80 rounded-xl p-4 md:p-5 border border-slate-200 relative">
            {isSolving && (
              <div className="absolute inset-0 bg-indigo-50/60 z-10 flex items-center justify-center backdrop-blur-[1.5px] rounded-xl">
                <span className="text-sm font-bold text-indigo-600 animate-pulse tracking-widest">
                  魔法運算中...
                </span>
              </div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-4">
              <div>
                <label className="text-sm font-bold text-indigo-700 flex items-center gap-1.5">
                  <BarChart3 size={16} /> 1-5 年預估成長率 (%)
                </label>
                <span className="text-xs text-slate-500 flex items-center gap-1.5 font-semibold mt-1">
                  隱含年複合成長 (CAGR):{" "}
              ...
              <div className="flex items-center bg-white border border-indigo-200 rounded-lg pl-3 pr-1 py-1 shadow-sm">
                ...
                <input
                  ...
                  className="w-16 text-sm font-black text-indigo-600 bg-indigo-50 rounded px-2 py-1 text-center outline-none placeholder-indigo-300 focus:ring-2 focus:ring-indigo-500/20"
                  placeholder="10"
                />
              </div>
            </div>
              ...
              {growth1Rates.map((rate, idx) => (
                <div key={idx} className="flex flex-col items-center">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-200 rounded-lg py-2.5 text-sm font-black text-slate-700 text-center outline-none focus:bg-indigo-50 focus:border-indigo-400 focus:text-indigo-700 focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-inner shadow-slate-100"
                    placeholder="0"
                  />
                </div>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div
                key={idx}
                className="bg-slate-50/50 rounded-xl p-3 border border-slate-200 flex flex-col justify-center"
              >
                <label
                  className={`text-xs font-bold text-${item.color}-600 block mb-2 flex items-center gap-1.5 justify-center`}
                >
                ...
                <input
                  ...
                  className={`w-full bg-white border border-slate-200 rounded-lg px-3 py-2.5 text-sm font-black text-${item.color}-700 text-center outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/10 transition-all shadow-sm`}
                  placeholder="0"
                />
              </div>
            ))}
        ...
        {/* --- 卡片 3：尊爵黑 估值結果 --- */}
        <section className="bg-slate-900 rounded-2xl shadow-xl shadow-slate-900/20 border border-slate-800 p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
          {/* 裝飾性光暈 */}
          ...
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4 pointer-events-none"></div>
          ...
          <div className="flex flex-col items-center justify-center gap-2 z-10">
            ...
            Math.abs(Number(valuation.fairValue) - Number(price)) < 1 ? (
              <span className="text-xs font-bold text-emerald-300 bg-emerald-500/20 px-4 py-1.5 rounded-full border border-emerald-500/30 flex items-center gap-1.5 shadow-lg shadow-emerald-900/20">
                <CheckCircle2 size={14} /> 現價完美匹配
              </span>
            ) : (
          ...
          <div className="flex flex-col text-center md:text-right z-10">
            <span className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">
              Upside Potential
            </span>
            <span
        ...
        <section className="space-y-4">
          <div className="flex justify-center">
            <div className="bg-slate-200/70 p-1.5 rounded-xl inline-flex shadow-inner">
              <button
              ...
              >
                <BarChart3 size={16} /> 成長視覺圖
              </button>
            </div>
          </div>

          {activeTab === "table" ? (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden animate-in fade-in slide-in-from-bottom-4">
              <div className="overflow-x-auto">
                <table className="w-full text-right text-sm">
                  <thead className="bg-slate-50 text-slate-500 text-xs font-bold border-b border-slate-200">
                    <tr>
                      <th className="px-5 py-4 text-center w-20 tracking-wider">
                        年度
                      </th>
                      <th className="px-5 py-4 tracking-wider">
                        每股參數 (EPS)
                      </th>
                      <th className="px-5 py-4 tracking-wider">成長率</th>
                      <th className="px-5 py-4 text-emerald-600 tracking-wider">
                        折現值 (PV)
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-slate-700 font-mono">
                    {valuation.path.map((row) => (
                      ...
                      >
                        <td className="px-5 py-3.5 text-center font-bold text-slate-800 bg-slate-50/50 group-hover:bg-indigo-50/50 transition-colors">
                          {row.year}
                        </td>
                        <td className="px-5 py-3.5 font-medium">{row.eps}</td>
                        <td className="px-5 py-3.5 text-slate-400 text-xs">
                          {row.growth}%
                        </td>
                        <td className="px-5 py-3.5 text-emerald-600 font-bold bg-emerald-50/20">
                          {row.pv}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="bg-slate-800 text-white border-t border-slate-700 text-sm">
                    <tr>
                      <td
                        colSpan="3"
                        className="px-5 py-3 text-slate-400 font-medium"
                      >
                        第 11 年的每股參數預估
                      </td>
                      <td className="px-5 py-3 font-mono font-medium">
                        {valuation.summary.epsYear11}
                      </td>
                    </tr>
                    <tr>
                      <td
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200/60 h-80 flex flex-col justify-end animate-in fade-in zoom-in-95">
              <div className="grid grid-cols-10 gap-2 items-end h-64 w-full">
                {valuation.path.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="absolute -top-6 text-[10px] md:text-xs font-mono font-bold text-slate-500 opacity-0 group-hover:opacity-100 group-hover:-translate-y-2 transition-all duration-300">
                        {item.eps}
                      </div>
                      <div className="w-full px-1 flex items-end justify-center h-full">
                        <div
                          className={`w-full rounded-t-md transition-all duration-700 ease-out group-hover:brightness-110 ${
                            isStage1
                              ? "bg-gradient-to-t from-indigo-300 to-indigo-400 shadow-indigo-200"
                              : "bg-gradient-to-t from-slate-300 to-slate-400 shadow-slate-200"
                          } ${
                            parseFloat(item.eps) < 0
                      ...
                      <span className="text-[10px] md:text-xs font-bold text-slate-400 mt-3 group-hover:text-slate-700 transition-colors">
                        Y{item.year}
                      </span>
                    </div>
                  );
